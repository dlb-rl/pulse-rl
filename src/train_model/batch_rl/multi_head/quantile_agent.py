...
from src.train_model.batch_rl.multi_head import helpers
...
class QuantileAgent(rainbow_agent.RainbowAgent):
    ...
    def __init__(
        ...
        tf_device="/cpu:0",
        optimizer=tf.train.AdamOptimizer(learning_rate=0.00005, epsilon=0.0003125),
        summary_writer=None,
    ...
    def _build_target_distribution(self):
        ...
        next_logits = tf.gather_nd(
            self._replay_next_target_net_outputs.logits, batch_indexed_next_qt_argmax
        )
    ...
    def _build_train_op(self):
        ...
        bellman_errors = (
            target_distribution[:, None, :] - chosen_action_logits[:, :, None]
        )  # Input `u' of Eq. 9.
        huber_loss = tf.to_float(  # Eq. 9 of paper.
            tf.abs(bellman_errors) <= self.kappa
        ) * 0.5 * bellman_errors ** 2 + tf.to_float(
            tf.abs(bellman_errors) > self.kappa
        ) * self.kappa * (
        ...
        with tf.control_dependencies([update_priorities_op]):
            if self.summary_writer is not None:
                with tf.variable_scope("Losses"):
                    ...
                    tf.summary.scalar(
                        "mean_q", tf.reduce_mean(self._replay_net_outputs.q_values)
                    )
                    tf.summary.scalar(
                        "max_q", tf.reduce_max(self._replay_net_outputs.q_values)
                    )
                    tf.summary.scalar(
                        "min_q", tf.reduce_min(self._replay_net_outputs.q_values)
                    )
    ...
    def _select_action(self):
        ...
        q_values = self._sess.run(
            self._net_outputs.q_values, {self.state_ph: self.state}
        )

        if random.random() <= epsilon:
            # Choose a random action with probability epsilon.
            return random.randint(0, self.num_actions - 1), actions_prob, q_values
        else:
            # Choose the action with highest Q-value at the current state.
            return best_action, actions_prob, q_values
