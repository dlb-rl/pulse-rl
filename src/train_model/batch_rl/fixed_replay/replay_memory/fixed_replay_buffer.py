...
from src.train_model.batch_rl.fixed_replay.replay_memory import logged_replay_buffer
...
class FixedReplayBuffer(object):
    ...
    def _load_and_save_buffers(self, num_load=None, num_buffers=None):
        ...
        for idx in range(num_load_steps):
            ...
            with futures.ThreadPoolExecutor(
                max_workers=num_load
            ) as thread_pool_executor:
                print("Ckpt suffixes: ", ckpt_indices)
                replay_futures = [
                    thread_pool_executor.submit(self._load_and_save_buffer, suffix)
                    for suffix in ckpt_indices
                ]
                for f in replay_futures:
                    replay_buffer = f.result()
