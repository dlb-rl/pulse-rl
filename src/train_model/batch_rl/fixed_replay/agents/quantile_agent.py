...
from src.train_model.batch_rl.fixed_replay.replay_memory import fixed_replay_buffer
from src.train_model.batch_rl.multi_head import quantile_agent
import gin
...
class FixedReplayQuantileAgent(quantile_agent.QuantileAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        print("Creating FixedReplayAgent with replay directory: ", replay_data_dir)
        print("\t init_checkpoint_dir: ", init_checkpoint_dir)
        print("\t replay_suffix ", replay_suffix)
        self._replay_data_dir = replay_data_dir
        self._replay_suffix = replay_suffix
        if init_checkpoint_dir is not None:
            self._init_checkpoint_dir = os.path.join(init_checkpoint_dir, "checkpoints")
        else:
            self._init_checkpoint_dir = None

        print("Created Quantile Agent....")
        super(FixedReplayQuantileAgent, self).__init__(
            sess, num_actions, observation_shape, observation_dtype, **kwargs
        )
